...
router.get("/single", verifyToken, (req, res) => {
  let userImage = req.user.img[0];
  const data = {
    userImage: userImage,
  };
...
router.get("/book-service",verifyToken, async (req, res) => {
try{
...
router.get("/gallery", verifyToken, async (req, res) => {
  ...
  try {
    ...
    return res.render("gallery", { data: { galleries } });
  } catch (error) {
  ...
  try {
    ...
    if (user) {
      ...
      if (isPasswordMatch) {
        ...
        return res
          .status(200)
          .json({ message: "Login successful", isVery: 2, isSuccess: true });
      } else {
        return res
          .status(400)
